...
class PredictionResponse(BaseModel):
    accepted: bool
    duplicate: bool
    same_payload: bool | None = None


class StatusResponse(BaseModel):
    next_index: int
    outstanding_index: int | None
    accepted_count: int
    frame_count: int
    recent_state_size: int
...
def create_app(settings: MockSettings | None = None) -> FastAPI:
    ...
    @app.get("/api/frames/next", response_model=list[FrameMetadata])
    async def get_next_frame(request: Request) -> list[FrameMetadata]:
    ...
    async def post_prediction(
        request: Request,
        payload: Annotated[list[Prediction], Body()],
    ) -> PredictionResponse:
