...
from .vision import get_shared_vision

if TYPE_CHECKING:
    from .config import ClientSettings


LOGGER = logging.getLogger("hurgor.inference")
...
class PipelineInferenceEngine:
    ...
    _vision: object | None = field(default=None, init=False, repr=False)
    _class_name_to_id: dict[str, int] = field(default_factory=dict, init=False, repr=False)

    @classmethod
    def from_settings(cls, settings: ClientSettings) -> PipelineInferenceEngine:
        engine = cls()
        model_path = settings.yolo_onnx_path or "models/hurgor_final.onnx"
        engine._vision = get_shared_vision(model_path)
        names = getattr(getattr(engine._vision, "model", None), "names", {})
        if isinstance(names, dict):
            engine._class_name_to_id = {str(name): int(class_id) for class_id, name in names.items()}
        return engine
    ...
    def infer_timed(
        ...
        *,
        ...
        if self._vision is not None:
            vision_frame = np.asarray(image)[:, :, ::-1].copy()
            raw_detections = self._vision.run_inference(vision_frame, frame_id=1)
            base_url = str(frame.url).rstrip("/").rsplit("/frames/", 1)[0]
            for detection in raw_detections:
                ...
                if class_id is None or not isinstance(bbox, (list, tuple)) or len(bbox) != 4:
                    continue
                ...
                objects.append(
                    DetectedObject.from_class_id(
                        class_id,
                        base_url=base_url,
                        top_left_x=max(0.0, center_x - width / 2.0),
                        top_left_y=max(0.0, center_y - height / 2.0),
                        bottom_right_x=max(0.0, center_x + width / 2.0),
                        bottom_right_y=max(0.0, center_y + height / 2.0),
                    )
                )
        elif degraded and hasattr(self.object_detector, "detect_fast"):
        ...
        LOGGER.info(
            "Frame processed frame=%s detections=%d undefined=%d inference_ms=%.3f",
            frame.url,
            len(objects),
